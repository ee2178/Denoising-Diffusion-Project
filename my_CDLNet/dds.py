...
class DDS(nn.Module):
    ...
    def prep_noise_schedule(self, mode = 'linear', sigma_start = 1.)
        ...
        if mode == 'linear':
            ns = torch.linspace(sigma_start, self.sigma_L, int(self.sigma_L*100/2))
        if mode == 'eero':
            ns = [sigma_start]
            i=1
    ...
    def forward(self, y, noise_level, acceleration_map, smaps, save_dir = None, verbose = True, sched = 'linear', maxit = 1e4, tol=1e-3):
        ...
        sig_t_vec = prep_noise_schedule(sched)

        # Compute alphas from sigmas
        alpha_bar_vec = 1/(1+sig_t_vec**2)
        alpha = torch.zeros(alpha_bar_vec.shape)
        alpha[0] = alpha_bar_vec[0] # First elements are the same, following elements are ratios of consecutive entries
        alpha[1:] = alpha_bar_vec[1:]/alpha[:-1]
        ...
        x_hat_t, _ = self.denoiser(x_t, sigma_L*255.)
        x_t = x_t + sigma_L**2*(x_hat_t-x_t)
        return x_t
