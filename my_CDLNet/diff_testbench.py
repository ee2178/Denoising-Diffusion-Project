...
def eval_immap( immap,      # ImMAP class
    ...
    if save == True:
        saveimg(out, img_dir+"/slice_"+str(n_slice)+"/immap"+str(mode)+"out.png", contrast=True)
        saveimg(err, img_dir+"/slice_"+str(n_slice)+"/immap"+str(mode)+"err.png", contrast=True)
    return out
...
def main():
    ...
    net_immap2p5 = train.load_model('trained_nets/'+organ+'/LPDS_ImMAP2p5_noSSDUMasking_sigmaScaleON_noiseUNIFORM'+'/args.json', device = device)
    # net_immap2p5 = train.load_model('trained_nets/'+organ+'/LPDS_ImMAP2p5_SSDU_R'+str(accel)+'_baseAccel1x_sigmaScaleON_noiseUNIFORM/args.json', device = device)
    ...
    e2e_recon, _ = lpdsnet_e2e(meas_kspace, 0.001, mask = mask, smaps = smaps, mri = True)

    # Save the e2erecon for comparison
    if save_extra is True:
        saveimg(gnd_truth, "immap_figs/slice_"+str(slice_)+"/gndtruth.png", contrast=True)
        saveimg(e2e_recon, "immap_figs/slice_"+str(slice_)+"/e2erecon.png", contrast=True)
        # Save lpdsnet error
        saveimg(brain_mask*(e2e_recon-gnd_truth)*5, "immap_figs/slice_"+str(slice_)+"/e2erecon_err.png", contrast=True)
    # Init ImMAP class
    # We may want to try a bunch of different lambda values:
    immap = ImMAP(net, lam = 2, beta = 0.05, sigma_L = 0.01)
    dds = DDS(net)
    # Generate brain mask 
    modes = ['2.5']
    # modes = ['2', '2-WS']
    ...
    # save_dir = None
    immap_outs.append([gnd_truth, e2e_recon])
    for mode in modes:
        immap_outs.append(eval_immap(immap, dds, slice_, meas_kspace, whitened_kspace, smaps, noise_level, mask, brain_mask, mode, gnd_truth, net_immap2p5, e2e_recon, save=True, save_dir = save_dir)) 
