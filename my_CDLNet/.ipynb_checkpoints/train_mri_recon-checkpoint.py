...
def fit(net, opt, loaders,
    ...
    while step < max_steps:
        ...
        elif loss_type == 'sigma-scaled-complex-mse' and x_init is False:
            # Generally speaking invalid for x_init false
            loss = torch.sum((sig_t+1e-2)**(-2)*(batch-batch_hat).abs()**2)/denom
        elif loss_type == 'sigma-scaled-magnitude-mse' and x_init is False:
            loss = torch.sum((sig_t+1e-2)**(-2)*(batch.abs()-batch_hat.abs())**2)/denom
        loss.backward()
                    ...
                    if itern == 0:
                        ...
                        if x_init is True:
                            ...
                            wandb.log({
                                ...
                                panel,
                                caption="Ground Truth | Reconstruction | Initialization"
                            )
                            }, step=step)
                        else:
